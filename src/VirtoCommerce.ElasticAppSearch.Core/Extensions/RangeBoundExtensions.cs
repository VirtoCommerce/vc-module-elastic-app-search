...
public static class RangeBoundExtensions
{
    private delegate bool TryParseDelegate<T>(string value, out T result);

    public static bool TryParseFrom(bool include, string value, out RangeBound<double> bound)
    {
    ...
    public static bool TryParseFrom(bool include, string value, out RangeBound<DateTime> bound)
    {
        return TryParse(include, value, TryParseNullableDateTime, _ => true, out bound);
    }

    public static bool TryParseTo(bool include, string value, out RangeBound<double> bound)
    {
        return TryParse(include, value, TryParseNullableDouble,
            result => result == null || !double.IsNaN(result.Value) && double.IsFinite(result.Value) && (!include || result < double.MaxValue), out bound);
    }

    public static bool TryParseTo(bool include, string value, out RangeBound<DateTime> bound)
    {
        return TryParse(include, value, TryParseNullableDateTime, _ => true, out bound);
    }

    private static bool TryParse<T>(bool include, string value, TryParseDelegate<T?> tryParse, Func<T?, bool> isValid, out RangeBound<T> bound)
        where T : struct
    {
        bound = null;
